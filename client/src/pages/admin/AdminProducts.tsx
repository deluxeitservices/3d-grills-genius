...
import { Plus, Pencil, Trash2, Search, Upload, Loader2, ImageIcon, X, GripVertical } from "lucide-react";
              ...
              return (
                <tr key={product.id} className="border-b border-zinc-800/50 text-zinc-300 hover:bg-zinc-800/30">
                  <td className="py-2 px-3">
                    {product.images?.[0] ? (
                      <img src={product.images[0]} alt="" className="w-12 h-12 object-cover bg-zinc-800" />
                    ) : (
        ...
        <DialogContent className="bg-zinc-900 border-zinc-800 text-white rounded-none max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Product description..." className="bg-zinc-800 border-zinc-700 text-white rounded-none min-h-[100px]" data-testid="input-product-description" />
            </div>

            <div className="space-y-3">
              <Label className="text-zinc-400">Product Images *</Label>
            ...
            <div className="border-t border-zinc-800 pt-4 space-y-3">
              <Label className="text-zinc-400 font-medium block">Product Info Tabs</Label>
              <div className="space-y-2">
                <Label className="text-zinc-500 text-xs">How It Works</Label>
                <Textarea value={form.howItWorks} onChange={(e) => setForm({ ...form, howItWorks: e.target.value })} placeholder="Steps for ordering custom grillz..." className="bg-zinc-800 border-zinc-700 text-white rounded-none min-h-[60px]" />
              </div>
              <div className="space-y-2">
                <Label className="text-zinc-500 text-xs">Shipping Info</Label>
                <Textarea value={form.shippingInfo} onChange={(e) => setForm({ ...form, shippingInfo: e.target.value })} placeholder="Shipping details..." className="bg-zinc-800 border-zinc-700 text-white rounded-none min-h-[60px]" />
              </div>
              <div className="space-y-2">
                <Label className="text-zinc-500 text-xs">Return & Exchanges</Label>
                <Textarea value={form.returnExchanges} onChange={(e) => setForm({ ...form, returnExchanges: e.target.value })} placeholder="Return policy..." className="bg-zinc-800 border-zinc-700 text-white rounded-none min-h-[60px]" />
              </div>
            </div>

            <div className="border-t border-zinc-800 pt-4">
              <Label className="text-zinc-400 font-medium text-sm mb-3 block">SEO</Label>
              <div className="space-y-2">
            ...
            <Button type="submit" disabled={uploading || createMutation.isPending || updateMutation.isPending} className="w-full bg-red-600 hover:bg-red-700 rounded-none" data-testid="button-save-product">
